/*
  ...
  Modified by MediaTek Labs for LinkIt 7697 HDK
*/
...
bool SoftwareSerial::listen()
{
    ...
    if (active_object != this)
    {
        ...
        // by default the attachInterrupt() function
        // use a 50ms debounce - use special version of API instead
        // to remove the debounce setting.
        if(_inverse_logic)
            //Start bit high
            attachInterruptWithDebounce(_receivePin, handle_interrupt, RISING, 0);
        else
            //Start bit low
            attachInterruptWithDebounce(_receivePin, handle_interrupt, FALLING, 0);
...
{
    // This recv() function will be called in interrupt context.
...
void SoftwareSerial::setRX(uint8_t rx)
{
    if (!_inverse_logic)
    {
      ...
      // inverse (pull-down when idle) logic
      digitalWrite(rx, LOW);
      pinMode(rx, INPUT);
      digitalWrite(rx, LOW);
    }
...
void SoftwareSerial::begin(long speed)
{
    ...
    const uint32_t bit_delay = (float(1)/speed)*1000000;

    _tx_delay = bit_delay;

    // Only setup rx when we have a valid PCINT for this pin
    if (pin_has_eint(get_arduino_pin_desc((arduino_pin_t)_receivePin))) {
...
size_t SoftwareSerial::write(uint8_t b)
{
    ...
    // Write the start bit
    digitalWrite(_transmitPin, inv ? HIGH : LOW);
    ...
    digitalWrite(_transmitPin, inv ? LOW : HIGH);
