...
public:
    ...
    RHGenericSPI(RHSPIFrequency frequency = Frequency1MHz, RHSPIBitOrder bitOrder = BitOrderMSBFirst, RHSPIDataMode dataMode = DataMode0);
    ...
    virtual void setFrequency(RHSPIFrequency frequency);

protected:
    /// The configure SPI Bus frequency, one of RHGenericSPI::Frequency
    RHSPIFrequency    _frequency; // Bus frequency, one of RHGenericSPI::Frequency

    /// Bit order, one of RHGenericSPI::BitOrder
    RHSPIBitOrder     _bitOrder;

    /// SPI bus mode, one of RHGenericSPI::DataMode
    RHSPIDataMode     _dataMode;
};
#endif
